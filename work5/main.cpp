...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    tie(nodes, edges) = read_graphml("moscow_road_network.graphml");
    //  print_edges(edges);
      // 2. Задаём названия улиц для начала и конца маршрута
    Coord start, end;
    string name;
    tie(start, end, name) = edges[edges.size() - 1];
    string start_street_query = "Таганская улица";      // Название улицы для старта
    string end_street_query = "улица Обручева";      // Название улицы для финиша
